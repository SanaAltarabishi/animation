...
import 'package:animation/common/ui_scaler.dart';
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Stack(
        children: [
          TopLeft(
            //import package extra alignment
            child: UiScaler(
...
class _TitleText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Widget content = Column(
      ...
      //which means the column will take up the minimum vertical space required by its children.
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Gap(20),
        //It creates a vertical gap between the top of the column and the next row of children.
        Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Transform.translate(
              //widget is used to apply a translation (movement) to its child widget
              //! offeset in this code
              offset: Offset(-(TextStyles.h1.letterSpacing! * .5), 0),
              //The Offset class represents a 2D coordinate and takes an x and y value to specify the translation.
    ...
    return Consumer<FragmentPrograms?>(
      builder:(context, fragmentPrograms, _) {
        ...
        return TickingBuilder(builder:(context, time) {
          return AnimatedSampler((image, size, canvas) { const double overdrawPx = 30;
...
/* the _DifficultyBtn class represents a button widget that provides focus and hover handling. 
It displays a customizable label and can have different visual states based on its selected, 
isHovered, and isFocused properties.*/
...
The button is not selected (!selected) and either:
The button is being hovered over (state.isHovered is true).
The button has focus (state.isFocused is true).
                  */
                ...
                //   ),
                // ),
//if this true it will add contianer
  ...
  Widget build(BuildContext context) {
    return FocusableControlBuilder(
      ...
      builder: (_, state) {
        if ((state.isHovered || state.isFocused) &&
            _wasHovered &&
            _btnAim?.status != AnimationStatus.forward) {
          _btnAim?.forward(from: 0);
        }
        ...
        return SizedBox(
          ...
          child: Stack(children: [
            ...
            Center(
              ...
              .animate(autoPlay: false, onInit: (c) => _btnAim = c)
